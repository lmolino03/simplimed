...
class seccionado(Base_Assembler):
    ...
    def __generate_response(self, prompt, data):
            if self.price_flag:
    ...
    def run(self):
        ...
        prompts = get_prompts("", self.output_format, _type="simplify")
        ...
        for ingreso in tqdm(self.ingresos):
            ...
            secciones = apartados.split(",")

            for i,s in enumerate(secciones):
                secciones[i] = secciones[i].strip()


            keys_apartados = self.ingresos[ingreso].informes['alta'].secciones.keys()

            for i, zipp in enumerate(zip(prompts, secciones, keys_apartados)):
                prompt, section, section_key = zipp
                ...
                generado = self.__generate_response(prompt, data)
